...
def load_and_cache_examples(save_cache_path,dataDeal,filePath,data_name = 'DuIE',data_type='train'):
    cached_features_file = os.path.join(save_cache_path, 'cached_-{}_{}'.format(data_name,data_type))
    if os.path.exists(cached_features_file):
        ...
        features = dataDeal.get_inputsFeatures_json(filePath)
...
#训练模型
def train(trainLoader,bertMDPN,specialIndexs,num_train_epochs,device,devLoader,dev_epoch_num = 1,do_dev = True,
          save_step = 500,save_path = 'outputs/mdpn_output',
    ...
    dsaam_parameters = bertMDPN.dsaam.named_parameters()
    ...
    optimizer_grouped_parameters = [
        ...
        {"params": [p for n, p in dsaam_parameters if not any(nd in n for nd in no_decay)],"weight_decay": weight_decay, 'lr': 0.001},
        {"params": [p for n, p in dsaam_parameters if any(nd in n for nd in no_decay)], "weight_decay": 0.0, 'lr': 0.001},
...
if __name__ == '__main__':
    ...
    bertMDPN = BertMDPNForNer.from_pretrained(pretrainPath, config=config, dsaam_in_hidden_dim=config.hidden_size,
    ...
    train(trainLoader,bertMDPN,specialIndexs,30,device,devLoader)
